...
export default function AdminAgendamento() {
  ...
  const handleAddService = async (e) => {
    ...
    setNewService({ name: '', price: '', duration_minutes: '30', category: 'Geral' });
    fetchData();
  };
      ...
      {activeTab === 'services' && (
        <div className="space-y-6">
          ...
          <section className="space-y-2">
            ...
            {services.map((srv) => (
              <div key={srv.id} className="bg-gray-900 p-3 rounded-xl border border-gray-800 flex justify-between items-center text-xs">
                <div>
                ...
                <div className="flex space-x-1.5">
                  <button onClick={() => setEditingService(srv)} className="bg-blue-600/20 text-blue-400 p-1.5 rounded-lg font-bold border border-blue-500/30">✏️ Editar</button>
      ...
      {editingService && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <form onSubmit={handleUpdateService} className="bg-gray-900 w-full max-w-sm rounded-2xl p-5 border border-blue-500/40 space-y-3">
            ...
            <div className="flex space-x-2"><button type="button" onClick={() => setEditingService(null)} className="w-1/2 bg-gray-800 py-2 rounded-lg text-xs">Cancelar</button><button type="submit" className="w-1/2 bg-blue-600 py-2 rounded-lg text-xs font-bold text-white">Atualizar</button></div>
          </form>
        </div>
      )}
